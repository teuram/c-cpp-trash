...
void emit_color(int flags, int r, int g, int b) {
    ...
    if ((flags & FLAG_MODE_256) == 0) {
        ...
        file_out.open(hash, std::ios_base::app);
        file_out << (bg ? "\x1b[48;2;" : "\x1b[38;2;") << r << ';' << g << ';' << b << 'm';
        file_out.close();
        return;
    }
    ...
    file_out << (bg ? "\x1B[48;5;" : "\u001B[38;5;") << color_index << "m";
    file_out.close();
}
...
void emitCodepoint(int codepoint) {
    ...
    if (codepoint < 128) {
        ...
        file_out << (char) codepoint << std::endl;
        file_out.close();
    } else if (codepoint < 0x7ff) {
        std::cout << (char) (0xc0 | (codepoint >> 6));
        std::cout << (char) (0x80 | (codepoint & 0x3f));

        file_out << (char) (0xc0 | (codepoint >> 6));
        file_out << (char) (0x80 | (codepoint & 0x3f));
        file_out.close();
        ...
        file_out << (char) (0xe0 | (codepoint >> 12));
        file_out << (char) (0x80 | ((codepoint >> 6) & 0x3f));
        file_out << (char) (0x80 | (codepoint & 0x3f));
        file_out.close();
        ...
        file_out << (char) (0xf0 | (codepoint >> 18));
        file_out << (char) (0x80 | ((codepoint >> 12) & 0x3f));
        file_out << (char) (0x80 | ((codepoint >> 6) & 0x3f));
        file_out << (char) (0x80 | (codepoint & 0x3f));
        file_out.close();
    } else {
...
std::string hash_file(std::string file_name, int w, int h) {
    std::ifstream rfile;
...
void read_pic(std::string file_name, int w, int h) {
    std::ifstream rfile;
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-0") {
            ...
            if (std::__fs::filesystem::is_directory(arg)) {
                for (auto & p : std::__fs::filesystem::directory_iterator(arg)) {
                    if (std::__fs::filesystem::is_regular_file(p.path())) {
    ...
    if (mode == FULL_SIZE || (mode == AUTO && file_names.size() == 1)) {
        for (unsigned int i = 0; i < file_names.size(); i++) {
            ...
            s = home + "/.cache/tiv/";
            mkdir(s.c_str(), 700);
            s += hash_file(file_names[i], maxWidth, maxHeight);
            hash = s;

            namespace fs = std::__fs::filesystem;
            fs::path cached_file{ s };

            if (fs::exists(cached_file)) {
                read_pic(s, maxWidth, maxHeight);
                continue;
            }
